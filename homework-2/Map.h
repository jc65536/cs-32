...
inline bool Map::contains(const KeyType &key) const {
    int pos = findFirstAtLeast(key);
    return pos < m_size && m_data[pos].m_key == key;
}

inline bool Map::insert(const KeyType &key, const ValueType &value) {
    return doInsertOrUpdate(key, value, true /* insert */, false /* no update */);
}

inline bool Map::update(const KeyType &key, const ValueType &value) {
